...
/** Android・iPhoneなどモバイル用追加コード **/
var agent = window.navigator.userAgent.toLowerCase();

/// モバイルかどうかを判定するフラグ
var isMobile = agent.match(/android/i)
  || agent.match(/webos/i)
...
var zoomOnly = true;
/// lightboxの画像ラップ要素
var ltbxImgWrap = $('#lightbox');
/// lightboxで表示される画像要素
var ltbxImg = ltbxImgWrap.find('img');
/// lightboxの画像表示時の黒い背景要素
var ltbxOverlay = $('#lightboxOverlay');
var timeIdForPos;
/// Y移動量合計（下フリックでの画像閉じに利用）
var sumOfDiffY = 0;

/// 画像下フリック閉じはiPhone系統でも有効化
if(isMobile
  || agent.match(/iphone/i)
  || agent.match(/ipad/i)
  || agent.match(/ipod/i)
...
$(document).on('touchmove', '#lightbox', function(e){
  ...
  if(touches.length === 1){
    ...
    if(1 <= scale && scale <= 1.2){
      var diffY = (pageCoord.y !== 0)
        ? pageY - pageCoord.y : 0;
      sumOfDiffY += diffY / scale;
      /// Y移動量合計が画像高さ1/3以上なら閉じる
      if(sumOfDiffY > ltbxImg.height()*1/3){
        $('.lb-close').click();
      }
      pageCoord.x = pageX;
      pageCoord.y = pageY;
...
$(document).on('touchstart', '[data-lightbox] img', function(){
  /// 画像が開かれたら各種パラメーター初期化
  scale = 1;
  pos = {x:0, y:0};
  ltbxImgWrap.css({
...
$(document).on('touchmove', '#lightbox', function(e){
  ...
  if(touches.length === 1){
    ...
    var pageY = touches[0].pageY ;
    //if(scale == 1){ return; }
    if(1 <= scale && scale <= 1.2){ return; }

    if(pageCoord.x !== 0 || pageCoord.y !== 0){
      var diffX = pageX - pageCoord.x;
      var diffY = pageY - pageCoord.y;
      posPrev.x = pos.x;
      posPrev.y = pos.y;
      pos.x += diffX / scale;
      pos.y += diffY / scale;
...
}).on('touchend', ltbxImg, function(e){
  /// lightbox画像がタッチされたら座標リセット
  var touches = e.originalEvent.changedTouches ;
  if(pageCoord.x !== 0 && pageCoord.y !== 0){
    pageCoord.x = 0;
    pageCoord.y = 0;
...
$(document).on('touchmove', function(e){
  ...
  /// タッチ操作を切り替え処理
  /// オーバーレイが出ていればタッチ操作無効にする
  if(ltbxOverlay.css('display') == 'none'){
  ...
  if ( touches.length > 1 ) {
    ...
    var scaleDelta 
      = 1-(Math.max(0.9, baseDistance / distance));

    scale += scaleDelta * scale * 1.5;
    if(scale < 1){ scale = 1; }
    update();
